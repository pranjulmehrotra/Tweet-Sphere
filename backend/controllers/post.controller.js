...
export const  createPost= async (req,res)=>{
 ...
 export const commentOnPost = async (req, res) => {
    try {
        ...
        post.comments.push(comment); // Ensure the comment is added to the `comments` array
        await post.save();
...
export const getlikedPosts = async (req, res) => {
    try {
        ...
        const likedPosts = await Post.find({ _id: { $in: user.likedPosts } })
            .populate({
                path: "user",
                select: "-password", // Exclude password from the user field
            })
            .populate({
                path: "comments.user",
                select: "-password", // Exclude password from comment user field
            });
...
export const getUserPosts= async (req,res)=>{
    try {
        ...
        console.log("Error in getUserPosts contoller:",error);
        res.status(500).json({error:"Internal server error"});
